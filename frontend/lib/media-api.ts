...
const DEFAULT_PRESIGN_ERROR_MESSAGE = "メディアのアップロードURL取得に失敗しました";
    ...
    try {
        ...
        if (isApiError(error) && error.response) {
            ...
            if (status === 400 || status === 409) {
                const message = extractErrorMessage(data) ?? DEFAULT_PRESIGN_ERROR_MESSAGE;
...
export async function registerMedia(
    ...
    try {
        ...
        if (isApiError(error) && error.response) {
            const { status, data } = error.response;
            if (status === 400 || status === 409) {
                const message = extractErrorMessage(data) ?? DEFAULT_REGISTER_ERROR_MESSAGE;
                throw new Error(message);
            }
        }
        throw error;
...
export async function uploadMediaObject(
    ...
    if (typeof window === "undefined") {
        throw new Error("uploadMediaObject はクライアント環境でのみ使用してください");
    }
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        xhr.onload = () => {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                reject(new Error(`${DEFAULT_UPLOAD_ERROR_MESSAGE} (status: ${xhr.status})`));
            }
        };
